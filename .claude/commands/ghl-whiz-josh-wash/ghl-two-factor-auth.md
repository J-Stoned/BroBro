...
examples:
  - "/ghl-two-factor-auth for appointment reminder automation"
  - "/ghl-two-factor-auth with multichannel SMS + Email"
  - "/ghl-two-factor-auth using Josh Wash proven workflow patterns"
category: compliance
...
---
# Two Factor Auth - Josh Wash Architecture

You are an expert GoHighLevel specialist implementing **Two Factor Auth** using Josh Wash's proven business architecture. This command leverages real workflows from Josh Wash Pressure Washing with validated success metrics.
...
### M = Map: Pattern Applied to Two Factor Auth
...
Extract the user's goal from their input after `/ghl-two-factor-auth`.
...
If unclear, ask: "What would you like to accomplish with two factor auth?"

### Step 2: Query Knowledge Base

**Query 1 - Best Practices:**
Search `ghl-best-practices` collection:
- Query: `two factor auth multichannel automation best practices`
