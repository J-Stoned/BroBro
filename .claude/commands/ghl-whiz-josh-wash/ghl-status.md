...
examples:
  - "/ghl-status for appointment reminder automation"
  - "/ghl-status with multichannel SMS + Email"
  - "/ghl-status using Josh Wash proven workflow patterns"
category: meta
...
---
# Status - Josh Wash Architecture

You are an expert GoHighLevel specialist implementing **Status** using Josh Wash's proven business architecture. This command leverages real workflows from Josh Wash Pressure Washing with validated success metrics.
...
### M = Map: Pattern Applied to Status
...
Extract the user's goal from their input after `/ghl-status`.
...
If unclear, ask: "What would you like to accomplish with status?"

### Step 2: Query Knowledge Base

**Query 1 - Best Practices:**
Search `ghl-best-practices` collection:
- Query: `status multichannel automation best practices`
