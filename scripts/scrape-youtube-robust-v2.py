"""
BroBro - Robust YouTube Transcript Scraper v2
Extracts transcripts from ANY YouTube URL with multiple fallback methods
NOW WITH FULL YT-DLP SUBTITLE EXTRACTION TO BYPASS IP BLOCKS
"""
...
class RobustYouTubeScraper:
    ...
    def get_transcript_method1(self, video_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"  [Method 1] SUCCESS via youtube-transcript-api")
    ...
    def get_transcript_method2(self, video_id: str) -> Optional[Dict]:
        ...
        This method bypasses youtube-transcript-api IP blocks
        """
        if not HAS_YT_DLP:
            return None

        try:
            print(f"  [Method 2] Trying yt-dlp (IP block workaround)...")
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                # Try English subtitles first (manual captions preferred)
                subtitle_data = subtitles.get('en') or automatic_captions.get('en')

                if not subtitle_data:
                    print(f"  [Method 2] No English subtitles available")
                    return None

                # Find the best subtitle format to download
                subtitle_url = None
                for fmt in subtitle_data:
                    # Prefer json3 format for structured data
                    if fmt.get('ext') == 'json3':
                        subtitle_url = fmt.get('url')
                        break
                    # Fallback to any format with a URL
                    if not subtitle_url and fmt.get('url'):
                        subtitle_url = fmt.get('url')

                if not subtitle_url:
                    print(f"  [Method 2] No downloadable subtitle URL found")
                    return None
                ...
                if not entries:
                    print(f"  [Method 2] No text found in subtitles")
                    return None
                ...
                is_generated = 'en' in automatic_captions and 'en' not in subtitles

                print(f"  [Method 2] SUCCESS via yt-dlp (bypassed IP block!)")
    ...
    def scrape_video(self, url: str) -> Optional[Dict]:
        ...
        # Method 1: youtube-transcript-api (fastest but may be IP blocked)
        transcript_data = self.get_transcript_method1(video_id)

        # Method 2: yt-dlp (slower but bypasses IP blocks)
...
def main():
    if len(sys.argv) < 2:
        print("\nRobust YouTube Transcript Scraper v2")
        print("="*70)
        print("\nNEW: Now bypasses YouTube IP blocks using yt-dlp!")
        print("\nUsage:")
        print("  python scrape-youtube-robust-v2.py <youtube_url>")
        print("  python scrape-youtube-robust-v2.py <url1> <url2> <url3> ...")
        print("\nExamples:")
        print("  python scrape-youtube-robust-v2.py https://www.youtube.com/watch?v=VIDEO_ID")
        print("  python scrape-youtube-robust-v2.py https://youtu.be/VIDEO_ID")
    ...
    print("\n" + "="*70)
    print("BroBro - Robust YouTube Transcript Scraper v2")
    print("="*70)
